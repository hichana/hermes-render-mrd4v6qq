...
# script's process environment via with-contenv. This hook needs it for
# ${HERMES_HOME} below (a caller-supplied Render/docker env var) -- without
# with-contenv, DATA_DIR would silently fall back to its /opt/data default
# even when HERMES_HOME was set to something else.
#
...
# is no longer seeded at boot from a Render Environment-tab var -- that
# mechanism was insert-only and went silently stale on rotation. It's now
# set post-deploy via admin-tools/env-sync, run from an admin's machine.
#
