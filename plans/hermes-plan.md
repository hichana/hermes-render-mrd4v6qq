...
- [ ] **Understand the session-key model so you set the right isolation knob.** Hermes keys sessions like this:
  ...
  Note that this table covers the **chat-platform** door only. The OpenAI-compatible API server (`/v1/chat/completions`) carries identity completely differently — the caller asserts it — and whether that yields distinct Honcho peers is unverified. Irrelevant while every user reaches the agent through LINE/Slack; a blocker the moment we build our own UI. See `plans/api-server-identity-plan.md`.
- [ ] **If this business needs a custom UI (dashboard, portal) rather than only chat platforms, resolve `plans/api-server-identity-plan.md` first.** That plan's experiment is unrun, so per-user memory isolation on the API path is currently unknown — don't put real client users behind it until it's settled.
...
- [ ] If the API server is enabled for this business (`API_SERVER_ENABLED=true`), confirm `plans/api-server-identity-plan.md`'s experiment has been run and its answer is acceptable — and that whatever calls the API derives `X-Hermes-Session-Key` from its own authenticated session, never from client-supplied input. If the API server is off, tick this as N/A.
