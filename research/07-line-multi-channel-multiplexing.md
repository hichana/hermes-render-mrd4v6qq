...
> **Revised again, 2026-08-01 — Option A is built.** `plans/line-multi-channel-plan.md` turned this into `patches/line-multi-channel.patch` + `modules/line/line_multiplex.py`. Validated locally: all three LINE patches apply cleanly against a fresh clone and the real pulled image, `tests/gateway/test_line_plugin.py` passes in full (120 tests, including live cross-channel signature isolation via `_handle_webhook`) inside the actual pinned-tag runtime, and `scripts/smoke-test.sh` proves the same property against the real built image end to end. Two things this doc's original scoping didn't anticipate, found while building: (1) `BasePlatformAdapter.handle_message()` spawns a background task for the agent turn, so outbound sends can't rely on the inbound request's contextvar being current — routing had to add a `chat_id -> channel` map read at send time, not just the contextvar sketched in `08`; (2) the URL scheme changed from `08`'s `/p/{profile}/line/webhook` sketch to `/line/p/{profile}/webhook` after checking the real `caddy/Caddyfile`. **Not yet deployed** — Phase 0's live-instance empirical test and Phase 7's staged rollout are still pending, deliberately gated on explicit go-ahead since they touch the live client-facing instance. See the plan for current status.
